...
export const cityService = {
  async getAllCities(params?: {
    ...
    const response = await apiClient.get<CitiesResponse>(`/common/city?${queryParams.toString()}`);
  ...
  async getCityById(id: string): Promise<City> {
    const response = await apiClient.get<City>(`/admin/cities/${id}`);
...
export const themeService = {
  async getAllThemes(params?: {
    ...
    const response = await apiClient.get<ThemesResponse>(`/admin/themes?${queryParams.toString()}`);
