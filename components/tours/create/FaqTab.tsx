...
interface FaqTabProps {
  faqs: FaqItem[];
  setFaqs: (value: FaqItem[]) => void;
}

export function FaqTab({ faqs, setFaqs }: FaqTabProps) {
  const addFaq = () => {
    const newOrder = faqs.length > 0 ? Math.max(...faqs.map((f) => f.order)) + 1 : 1;
    setFaqs([...faqs, { question: '', answer: '', order: newOrder }]);
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Add common questions and answers to help travelers learn more about this tour
              </CardDescription>
            </div>
